...
function imgui:hscrollbar(t)
	return scrollbar(self, t, false)
end

function imgui:vscrollbar(t)
	return scrollbar(self, t, true)
end

function imgui:scrollbox(t)
	local id = assert(t.id, 'id missing')
	local x, y, w, h = self:getbox(t)
	local cx = t.cx or 0
	local cy = t.cy or 0
...
function imgui:begin_scrollbox(t)
	local cx, cy, x, y, w, h = self:scrollbox(t)
	self:begin_box(w, h)
	self.cr:save()
...
if not ... then
	...
	local cx, cy = 0, 0
	local cw, ch = 2000, 1000
	function player:render()
		cx, cy =
		self:begin_scrollbox{id = 1, cw = cw, ch = ch, cx = cx, cy = cy}

		self:textbox(0, 0, cw, ch, 'Hello World!')
